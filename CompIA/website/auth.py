        ...
        if user:
            ...
            if flag==True:
                ...
                for j in [slot1,slot2,slot3]:
                    db.session.add(j)
                db.session.commit()
            newUser = User(email=email, password=generate_password_hash(password1,method="sha256"), name=firstName, org_id= to_send.id, days=8)
...
def accessSubjects():
	conn = sqlite3.connect("examBase.db") # connect to database
	cursor = conn.cursor()   # creates a cursor
	cursor.execute("SELECT subject, option, paperonehl, papertwohl, paperthreehl FROM subjects")
	listOfTuples = cursor.fetchall()
	conn.commit()  #commit comand
	conn.close()
	return listOfTuples