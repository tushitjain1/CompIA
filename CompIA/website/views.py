...
def home():
    ...
    # If GET request is recieved, variables are reset
    ...
    # If POSt request is recieved, data is processed
...
def calculate():
    ...
    for i in subsToPass:
        ...
        for k in timeSlots:
            ...
            if timeCalc(k.timeStart,k.timeEnd) >= (i[3])/60:
                temp2.append(k)                          
            if timeCalc(k.timeStart,k.timeEnd) >= (i[4])/60:
                temp3.append(k)
        x = exams(i[0],[temp1,[0]*(len(temp1))],i[1],i[2],1) #new object exam (name,domain,option,duration,papernumber)
    ...
    for slot in timeSlots:
        if (slot.timeEnd-slot.timeStart) > (biggestSlot.timeEnd-biggestSlot.timeStart):
            biggestSlot = slot
    for objects in objectList:
        for compared in objectList:            
            ...
            if compared.name == objects.name:
                if (compared.duration + objects.duration)/60 > (biggestSlot.timeEnd-biggestSlot.timeStart):
                    constraints.append((objects,compared))
    ...
    # If POST request is recieved from the HTML page, the timetable is saved
    if request.method == 'POST':
        ...
        # Adds the newly generatied timetable to the database
        for i in fc.solution:
        ...
        db.session.commit()
    return render_template('table.html',user=current_user, days=len(daysArray),times=htmlTimes,output=stringList,sol=sol)

# Funciton to calculate duration of a time slot
